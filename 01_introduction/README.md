...
建議用 Visual Studio Code，再安裝 [Go by Microsoft](https://marketplace.visualstudio.com/items?itemName=ms-vscode.Go)。因為使用到 [PlantUML](https://plantuml.com/)，需要安裝 JDK。
...
變數宣告後，它的資料型別也就固定，不能更改。不像 PHP/Python 可以隨意更改變數的資料型別。

### 3.3 可在不同平台執行

Go 可以實現跨平台，前題是沒使用到平台特有的函式庫。Go 的跨平台與 Java 不同，是需要重新 compile 成相對應的平台的 Machine Code。
...
與 Java 類似，有 GC (Garbage Collection)，可以不用自己管理記憶體，但也要注意誤用空指標與浪費記憶體。
...
- Go 1.18 以上支援泛型 (Generic)。可參考：[Tutorial: Getting started with generics](https://go.dev/doc/tutorial/generics)。

- 檔案的編碼，一定要是 __UTF-8__。
- 參數傳遞 (Paramenter Passing) 是 __Pass by Value__ (Call by Value)。
...
1. 同時開發多個專案時，可以使用 [go workspaces](https://go.dev/doc/tutorial/workspaces) 功能。
...
@import "ex01_01/main.go" {class="line-numbers"}
...
1. 寫執行檔的程式，檔名不一定要命名成 `main.go`，但建議使用 `main.go`，程式碼的 package 宣告一定要是 __main__。
...
1. 程式的進入點 (Entry point): `func main()`，跟大多數的程式語言一樣，寫執行檔都會需要有一個主函式 __main__

## 6. Arguemnts (ex01-02)

重覆上述的動作，sample code 如下：

@import "ex01_02/main.go" {class=line-numbers}
