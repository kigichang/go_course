...
Representational State Transfer (REST) 由 Roy Thomas Fielding 在 2000 年在 [Architectural Styles and the Design of Network-based Software Architectures](https://www.ics.uci.edu/~fielding/pubs/dissertation/top.htm) 這篇論文中提出。

REST 是一種設計風格，並非一種標準。通常使用 HTTP, URI, XML, HTML 來實作，近來 XML 的部分，改用了 JSON，或 XML 與 JSON 兩者並存。

## 2. Architectural Goals

RESTful 設計時，希望達到以下目的:
...
## 4. RESTful Interface Example

### 4.1 Use HTTP Response Codes to Indicate Status
...
### 4.2 Using HTTP Methods for RESTful Services
...
@import "restful/main.go" {class="line-numbers"}

#### 測試指令

1. List all categories:  `curl --include http://localhost:8080/categories`
1. Get one category:
    - success: `curl --include http://localhost:8080/categories/1`
    - failure: `curl --include http://localhost:8080/categories/100`
1. Add a category: `curl --include --request POST --header "Content-type: application/json" --data '{"name": "PC", "parent": 1}' http://localhost:8080/categories`
1. Update a category: `curl --include --request PUT --header "Content-type: application/json" --data '{"name": "NB-2", "parent": 1}' http://localhost:8080/categories/2`
1. Delete a category: `curl --include --request DELETE http://localhost:8080/categories/2`
